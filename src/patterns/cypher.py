...
logger = logging.getLogger('CypherPattern')

class CypherPattern(BaseHarmonicPattern):
    """
    Implementation of the Cypher harmonic pattern.
    
    The Cypher pattern is characterized by these Fibonacci ratios:
    - XA to AB: AB should be a 0.382-0.618 retracement of XA
    - BC to AB: BC should be a 1.272-1.414 extension of AB
    - CD to BC: CD should be a 0.786 retracement of BC
    - XA to AD: AD should be a 0.786 retracement of XA
    """
    
    def __init__(self, tolerance=0.05):
        """
        Initialize the Cypher pattern detector.
        
        Args:
            tolerance (float): Tolerance for Fibonacci ratio matching (default: 0.05)
        """
        super().__init__("Cypher", tolerance)
    
    def get_pattern_ratios(self):
        """
        Get the ideal Fibonacci ratios for the Cypher pattern.
        
        Returns:
            dict: Dictionary of ratios for each leg of the pattern
        """
        return {
            'AB_XA': [0.382, 0.618],  # AB should be between 0.382 and 0.618 retracement of XA
            'BC_AB': [1.272, 1.414],  # BC should be between 1.272 and 1.414 extension of AB
            'CD_BC': 0.786,  # CD should be a 0.786 retracement of BC
            'AD_XA': 0.786  # AD should be a 0.786 retracement of XA
        }
    
    def find_patterns(self, df, swing_window=5):
        """
        Find Cypher patterns in the given price data.
    ...
    def calculate_target_price(self, x_price, a_price, d_price):
        ...
        Returns:
            float: Target price
        """
        # For Cypher pattern, target is often the 1.27 extension of AD from D
        xa_move = abs(a_price - x_price)
        target_extension = 1.27

...
logger = logging.getLogger('GartleyPattern')

class GartleyPattern(BaseHarmonicPattern):
    """
    Implementation of the Gartley harmonic pattern.
    
    The Gartley pattern is characterized by these Fibonacci ratios:
    - XA to AB: AB should be a 0.618 retracement of XA
    - AB to BC: BC should be a 0.382-0.886 retracement of AB
    - BC to CD: CD should be a 1.272-1.618 extension of BC
    - XA to AD: AD should be a 0.786 retracement of XA
    """
    
    def __init__(self, tolerance=0.05):
        """
        Initialize the Gartley pattern detector.
        
        Args:
            tolerance (float): Tolerance for Fibonacci ratio matching (default: 0.05)
        """
        super().__init__("Gartley", tolerance)
    
    def get_pattern_ratios(self):
        """
        Get the ideal Fibonacci ratios for the Gartley pattern.
        
        Returns:
            dict: Dictionary of ratios for each leg of the pattern
        """
        return {
            'AB_XA': 0.618,  # AB should be a 0.618 retracement of XA
            'BC_AB': [0.382, 0.886],  # BC should be between 0.382 and 0.886 retracement of AB
            'CD_BC': [1.272, 1.618],  # CD should be between 1.272 and 1.618 extension of BC
            'AD_XA': 0.786  # AD should be a 0.786 retracement of XA
        }
    
    def find_patterns(self, df, swing_window=5):
        """
        Find Gartley patterns in the given price data.
        ...
        Returns:
            list: List of detected Gartley patterns with their details
        """
        ...
        # Look for bullish Gartley (X:low, A:high, B:low, C:high, D:low)
                ...
                for k in range(i + 1, len(swing_lows)):
                    ...
                    for l in range(j + 1, len(swing_highs)):
                        ...
                        for m in range(k + 1, len(swing_lows)):
                            ...
                            if not self.is_ratio_valid(ad_xa_ratio, ratios['AD_XA']):
                                continue
                            
                            # All ratios match, we found a bullish Gartley pattern
                            pattern_id += 1
                            patterns.append({
                                'id': pattern_id,
                                'type': 'Gartley',
        ...
        # Look for bearish Gartley (X:high, A:low, B:high, C:low, D:high)
                ...
                for k in range(i + 1, len(swing_highs)):
                    ...
                    for l in range(j + 1, len(swing_lows)):
                        ...
                        for m in range(k + 1, len(swing_highs)):
                            ...
                            # All ratios match, we found a bearish Gartley pattern
                            pattern_id += 1
                            patterns.append({
                                'id': pattern_id,
                                'type': 'Gartley',
        ...
        logger.info(f"Found {len(patterns)} Gartley patterns")
        return patterns 
...
logger = logging.getLogger('BatPattern')

class BatPattern(BaseHarmonicPattern):
    """
    Implementation of the Bat harmonic pattern.
    
    The Bat pattern is characterized by these Fibonacci ratios:
    - XA to AB: AB should be a 0.382-0.5 retracement of XA
    - AB to BC: BC should be a 0.382-0.886 retracement of AB
    - BC to CD: CD should be a 1.618-2.618 extension of BC
    - XA to AD: AD should be a 0.886 retracement of XA
    """
    
    def __init__(self, tolerance=0.05):
        """
        Initialize the Bat pattern detector.
        
        Args:
            tolerance (float): Tolerance for Fibonacci ratio matching (default: 0.05)
        """
        super().__init__("Bat", tolerance)
    
    def get_pattern_ratios(self):
        """
        Get the ideal Fibonacci ratios for the Bat pattern.
        
        Returns:
            dict: Dictionary of ratios for each leg of the pattern
        """
        return {
            'AB_XA': [0.382, 0.5],  # AB should be between 0.382 and 0.5 retracement of XA
            ...
            'AD_XA': 0.886  # AD should be a 0.886 retracement of XA
        }
    
    def find_patterns(self, df, swing_window=5):
        """
        Find Bat patterns in the given price data.
    ...
    def calculate_target_price(self, x_price, a_price, d_price):
        ...
        Returns:
            float: Target price
        """
        # For Bat pattern, target is often the 1.27 extension of AD from D
        xa_move = abs(a_price - x_price)

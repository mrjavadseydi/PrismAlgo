...
logger = logging.getLogger('CrabPattern')

class CrabPattern(BaseHarmonicPattern):
    """
    Implementation of the Crab harmonic pattern.
    
    The Crab pattern is characterized by these Fibonacci ratios:
    - XA to AB: AB should be a 0.382-0.618 retracement of XA
    - AB to BC: BC should be a 0.382-0.886 retracement of AB
    - BC to CD: CD should be a 2.618-3.618 extension of BC
    - XA to AD: AD should be a 1.618 extension of XA
    """
    
    def __init__(self, tolerance=0.05):
        """
        Initialize the Crab pattern detector.
        
        Args:
            tolerance (float): Tolerance for Fibonacci ratio matching (default: 0.05)
        """
        super().__init__("Crab", tolerance)
    
    def get_pattern_ratios(self):
        """
        Get the ideal Fibonacci ratios for the Crab pattern.
        ...
        return {
            ...
            'CD_BC': [2.618, 3.618],  # CD should be between 2.618 and 3.618 extension of BC
            'AD_XA': 1.618  # AD should be a 1.618 extension of XA
        }
    
    def find_patterns(self, df, swing_window=5):
        """
        Find Crab patterns in the given price data.
    ...
    def calculate_target_price(self, x_price, a_price, d_price):
        ...
        Returns:
            float: Target price
        """
        # For Crab pattern, target is often the 1.618 extension of AD from D
        xa_move = abs(a_price - x_price)
